...
def home():
    unique_interests = sorted(set(interest_mappings.values()))
    return render_template('index.html', interests=unique_interests)
...
# Route to get available interests
@app.route('/interests', methods=['GET'])
def get_interests():
    return jsonify({'interests': sorted(set(interest_mappings.values()))})
